...
author: "Andrew Norman, Charlie Head, and Muhammad Tahmid Chowdhury"
date: "2025-05-01"
output:
  pdf_document:
...
## 1. Motivation, load our packages and datasets, view the structures, and skim them.
...
Our topic of interest is predictive baseball analysis relating to next years team win ratio with the teams end of year statistics from the previous season. We are interested in predictive baseball analysis because of the film *Moneyball* in which the General Manager and assistant GM try to use data analytics to remove bias from their player scouting and selection to get winning team statistics from overlooked players due to many reasons on a limited budget. We do not have the same scouting and player level statistics that scouts and managers do, but using team level batting statistics we would like to try to model team's win ratios based off of end of season team batting statistics and payroll from the previous year since if you cannot hit and drive in runs then you cannot win games.
...
## 2. Merge our datasets into one and skim to check for missing values.
...
Our data set has 38 different variables containing the mentioned batting statistics, win statistics, and payroll statistics. Focusing on the key batting statistics we see that over the last 25 years: the average amount of runs per year is 719, the average number of homeruns is 171 per year, average batting average is .256 which is calculated by dividing total at bats by total base hits, average on base percentage is .325, average slugging percentage is .414 which is total bases divided by total at bats, average on base plus slugging percentage is .740, and average on base plus slugging percentage plus is 97.1. Aside from batting statistics, the average estimated team payroll was \$140 million. These are some of our key variables that will be helping us predict next season's win ratio off of end of year stats of the previous season.

All of our data was collected from baseball-reference.com for free use. Setting up our data included constructing multiple `.csv` files that included the relevant statistics we were looking for. We needed to make sure that we had all batting statistics, payroll estimates, win statistics, and one for inflation multipliers so our payroll data is all accurately adjusted and comparable. While constructing our final data set we decided to add a dummy for World Series winner each year as well. After merging all 4 data sets by year and team name, we then manually addressed `NA` values in the estimated payroll category by manually setting them to the proper values and renamed teams to current names (if a team moved cities at some point in the last 25 years there would be two different teams even though it is the same ball club). 
...
There is a clear relationship between estimated payroll and win ratio. As teams spend more money they are more likely to increase their win ratio. Notice how in the last 25 years there are only 4 teams that have spent more than \$250 million and had a win ratio lower than $.500$. All 4 teams that this is true for as well were very close to a $.500$ win ratio as well. There are teams that have achieved this for way less, but according to the statistics, wins can be bought to a degree. There are two outcomes that both results in success from the view of an owner that may differ from the views of fans. The first outcome is to win as many games as possible and have a good win ratio. As the team wins more that generates more interest from fans, which increases game attendance and therefore revenue. The second outcome is winning the World Series, which has the same effects as winning as many games as possible in the regular season, but to a far greater degree. The relationship for non-World Series winning teams has an $R^2$ of .13 and a strong correlation coefficient of $3 * 10^8$. This means that 13% of the variation in win ratio can be explained by the variation in explained payroll, and an increase in win ratio by .1  is associated with an increase in estimated payroll by \$30 million. Comparing these values to World Series winning teams and we see 22% of the variation in win ratio explained by the variation in estimated payroll and an increase in win ratio by .1 is associated with an increase in estimated payroll by \$42 million. Based off of this, World Series winning teams have to pay more for their wins than non winning teams, which makes sense since they are presumably paying more money for better players on their team to increase their odds of winning the World Series.
...
Our LASSO model seeks to predict the next season's win ratio based off the end of season batting statistics from the previous season including end of season payroll as well. A complete success would be the predicted values having very small residuals from the dashed black line with a slope of one. Our model did not achieve perfect results, but still shows a strong positive relationship in predicting the next season's win ratio. The difference of prediction vs actual most likely come from seasonal roster changes that happen due to trades, drafting, and moving players up and down from organizational minor league teams. The model cannot predict these shocks due to them being unpredictable in nature with only using year-end batting statistics. If this was attempted with individual player level statistics with multiple observations for each player over a year, it may be predictable that decreasing performance may lead to a trade but it would be unpredictable to where a player would go and who they would be replaced by. The variables that our LASSO model chose as the most important were: batters, batter age, average runs per game, runs, number of triples, home runs, batting average, stolen bases, OBP, OPS+, and estimated payroll. The three variables with the highest coefficients are runs, stolen bases, and OPS+. Estimated payroll and OBP have the next highest coefficients and are very important to the model too. This suggests that all the variables that were explored closer are impactful to a team's future success. Based on the RMSE values and final model, it looks like our LASSO model is underfitting since the slope of our predictions vs actual values is less than 1. If we wanted to improve our fitting on this model we could perhaps add more defensive statistics to our data and increase the amount of seasons in the data frame to give more data to test and train on.
